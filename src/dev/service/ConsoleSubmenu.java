...
public class ConsoleSubmenu {
    ...
    public static AbstractTask inputTask(TaskManager manager) {
                ...
                case "3":
                    ...
                    if (manager.containsTaskId(taskId)) {
                        if (manager.getTask(taskId) instanceof Epic) {
                            ...
                            String description = inputText(INPUT_DESCRIPTION_CAPTION);
                            Subtask subtask = epic.create(CollectionUtils.getNextTaskId(epic.keySet()), name, description);
                            System.out.println(INPUT_STATUS_CAPTION + epic.getTask(subtask.getTaskId()).getStatus().getTitle() + ".");
        ...
        while (true) {
            ...
            switch (command) {
                ...
                case "3":
                    ...
                    String description = inputText(INPUT_DESCRIPTION_CAPTION);
                    Subtask subtask = epic.create(CollectionUtils.getNextTaskId(epic.keySet()), name, description);
                    System.out.println(INPUT_STATUS_CAPTION + subtask.getStatus().getTitle() + ".");
                    out.println("Созданной подзадаче присвоен идентификационный номер: " + subtask.getTaskId());
                    return;
                case "4":
                    if (epic.size() == 0) {
